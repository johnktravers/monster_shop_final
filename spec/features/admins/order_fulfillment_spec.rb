...
RSpec.describe 'As an admin on a merchant order show page' do
  before :each do
    create_admin
  ...
  it 'can see customer info and info for only that merchants items' do
    visit "/admin/merchants/#{@mike.id}"

    within("#order-#{@order_2.id}") { click_link("#{@order_2.id}") }

    expect(current_path).to eq("/admin/merchants/#{@mike.id}/orders/#{@order_2.id}")
  ...
  it 'can fulfill an unfulfilled item, decreasing item inventory' do
    visit "/admin/merchants/#{@mike.id}/orders/#{@order_2.id}"
  ...
  it 'can see what coupon was used on the order if there is one' do
    ...
    visit "/admin/merchants/#{@mike.id}/orders/#{order_3.id}"
